...
import type { EditDescriptionBuilderProps } from "@/types/components/course/description-builder/edit-description-builder";
...
const EditDescriptionBuilder = ({ course }: EditDescriptionBuilderProps) => {
  ...
  const { mutate: updateCourseDescription, isPending } = useMutation({
    mutationFn: () =>
      updateCourse(course.id, { description: courseDescription }),
    onSuccess: ({ error }) => {
      if (error) {
        errorToast(error);
      } else {
...
export default EditDescriptionBuilder;
