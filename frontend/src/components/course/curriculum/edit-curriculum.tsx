...
export default function EditCurriculum({ initialCourse }: EditCurriculumProps) {
  ...
  const [addLectureTitle, setAddLectureTitle] = useState<string>("");
  const [lectureDialogOpen, setLectureDialogOpen] = useState<boolean>(false);
  // 섹션 추가 상태
  ...
  const addSectionMutation = useMutation({
    mutationFn: async (title: string) => {
      const { data, error } = await createSection(initialCourse.id, title);
  ...
  const addLectureMutation = useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await createLecture(sectionId, title);
  ...
  const updateSectionTitleMutation = useMutation({
    mutationFn: async ({
      ...
      return await updateSectionTitle(sectionId, title);
    },
    onSuccess: ({ error }) => {
      if (error) {
        errorToast(error);
  ...
  const toggleLecturePreviewMutation = useMutation({
    mutationFn: async (lecture: Lecture) => {
      const response = await updateLecturePreview(
        lecture.id,
        !lecture.isPreview
      );
    ...
    onSuccess: ({ error }) => {
      if (error) {
        ...
        toast.success("수업의 미리보기가 성공적으로 업데이트 되었습니다.");
  ...
  return (
    <>
      ...
      <LectureCreateDialog
        ...
        setAddLectureTitle={setAddLectureTitle}
